...
const baseSectionId: object = { sectionId: 0 };

export const SectionId = {
  encode(message: SectionId, writer: Writer = Writer.create()): Writer {
  ...
  decode(input: Reader | Uint8Array, length?: number): SectionId {
    const reader = input instanceof Uint8Array ? new Reader(input) : input;
    let end = length === undefined ? reader.len : reader.pos + length;
    const message = { ...baseSectionId } as SectionId;
  ...
  toJSON(message: SectionId): unknown {
    const obj: any = {};
    message.sectionId !== undefined && (obj.sectionId = message.sectionId);
    return obj;
  },

  fromPartial(object: DeepPartial<SectionId>): SectionId {
    const message = { ...baseSectionId } as SectionId;
    if (object.sectionId !== undefined && object.sectionId !== null) {
